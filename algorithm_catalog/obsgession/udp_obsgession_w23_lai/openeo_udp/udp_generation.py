...
import openeo
from eo_processing.config.settings import get_advanced_options, get_collection_options, get_job_options
from eo_processing.openeo.preprocessing import extract_S2_datacube
from openeo.api.process import Parameter
from openeo.processes import array_create, eq
...
def generate() -> dict:
    ...
    param_epsg = Parameter.number(name="epsg", description="The EPSG is set to 3035 by default.", default=3035)
    ...
    s2_cube = extract_S2_datacube(
        eoconn, param_AOI, param_start_date, param_end_date, **collection_options, **processing_options
    )

    LAI_cube = s2_cube.apply_dimension(
        dimension="bands", process=compute_LAI, context={"parallel": True, "TileSize": 128}
    ...
    tree_cube = eoconn.load_collection("ESA_WORLDCOVER_10M_2021_V2", spatial_extent=param_AOI, bands=["MAP"])
    tree_cube = tree_cube.resample_spatial(projection=param_epsg, resolution=resolution)
    ...
    return build_process_dict(
        process_graph=LAI_cube_mask_url.flat_graph(),
